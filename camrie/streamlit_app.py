...
if seq_path and os.path.exists(seq_path):
    try:
        ...
        fov_mm = seq_params["fov_mm"]
        # Flatten and convert to scalar floats (handle nested numpy arrays)
        fov_flat = np.array(fov_mm).flatten()
        default_seq_fov = (float(fov_flat[0]), float(fov_flat[1]))
    except:
        pass
...
# Sequence FOV
col1, col2 = st.sidebar.columns(2)
seq_fov_x = col1.number_input("Seq FOV X", value=float(default_seq_fov[0]))
seq_fov_y = col2.number_input("Seq FOV Y", value=float(default_seq_fov[1]))
...
save_nifti = st.sidebar.checkbox(
    "Save NIfTI volume",
    value=True,
    help="Assemble all slices into a properly-oriented NIfTI volume in body space"
)
save_dicom = st.sidebar.checkbox(
    "Export DICOM",
    value=False,
    help="Export slices as DICOM files with proper geometry tags"
)
    ...
    if st.button("🚀 Run Simulation", key="run_sim", type="primary"):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if save_nifti and os.path.exists(volume_path):
                    st.success(f"📦 NIfTI volume: `{volume_path}`")
                
                if save_dicom and os.path.exists(dicom_dir):
                    n_dcm = len([f for f in os.listdir(dicom_dir) if f.endswith('.dcm')])
                    st.success(f"📁 DICOM files: {n_dcm} slices in `{dicom_dir}`")
            else:
