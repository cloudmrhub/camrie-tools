...
def export_dicom_series(
    ...
    output_dir: str,
    patient_id: str = "SIMULATION",
    patient_name: str = "Phantom",
) -> None:
    """
    Export reconstructed slices as DICOM files.
    
    Creates DICOM-compliant images with proper geometry tags.
    ...
    # Generate UIDs
    study_uid = f"1.2.826.0.1.3680043.8.498.{uuid.uuid4().int >> 64}"
    series_uid = f"1.2.826.0.1.3680043.8.498.{uuid.uuid4().int >> 64}"
    frame_of_ref_uid = f"1.2.826.0.1.3680043.8.498.{uuid.uuid4().int >> 64}"
    
    now = datetime.now()
    date_str = now.strftime("%Y%m%d")
    time_str = now.strftime("%H%M%S")
    
    # Find global max for consistent scaling across all slices
    global_max = max(recon.max() for recon in recon_images if recon.size > 0)
    if global_max <= 0:
        global_max = 1.0
    
    for i, (recon, slice_spec) in enumerate(zip(recon_images, series_spec.slices)):
        # Convert to uint16 for DICOM (DICOM doesn't support floating point)
        # Scale to 0-65535 range
        recon_scaled = (recon / global_max * 65535).clip(0, 65535).astype(np.uint16)
        
        # Create SITK image
        img = sitk.GetImageFromArray(recon_scaled[None, :, :])
        
        # Set geometry
        nx, ny = recon.shape[1], recon.shape[0]
        spacing = series_spec.seq_fov_mm[0] / nx
        img.SetSpacing([spacing, spacing, series_spec.slice_thickness_mm])
        
        # Image position (patient) - slice corner
        R = slice_spec.R_body_to_seq.T
        corner_seq = np.array([
            -series_spec.seq_fov_mm[0] / 2.0,
            -series_spec.seq_fov_mm[1] / 2.0,
            0.0,
        ])
        corner_body = R @ corner_seq + slice_spec.center_mm
        
        # Image orientation (patient) - direction cosines
        row_dir = R[:, 0]  # x direction
        col_dir = R[:, 1]  # y direction
        
        # Set DICOM tags
        sop_uid = f"1.2.826.0.1.3680043.8.498.{uuid.uuid4().int >> 64}"
        
        img.SetMetaData("0008|0016", "1.2.840.10008.5.1.4.1.1.4")  # MR Image Storage
        img.SetMetaData("0008|0018", sop_uid)
        img.SetMetaData("0008|0020", date_str)
        img.SetMetaData("0008|0030", time_str)
        img.SetMetaData("0008|0060", "MR")
        img.SetMetaData("0010|0010", patient_name)
        img.SetMetaData("0010|0020", patient_id)
        img.SetMetaData("0018|0050", str(series_spec.slice_thickness_mm))
        img.SetMetaData("0020|000d", study_uid)
        img.SetMetaData("0020|000e", series_uid)
        img.SetMetaData("0020|0013", str(i + 1))  # Instance number
        img.SetMetaData("0020|0032", "\\".join(map(str, corner_body)))  # ImagePosition
        img.SetMetaData("0020|0037", "\\".join(map(str, list(row_dir) + list(col_dir))))  # ImageOrientation
        img.SetMetaData("0020|0052", frame_of_ref_uid)
        img.SetMetaData("0028|0010", str(ny))  # Rows
        img.SetMetaData("0028|0011", str(nx))  # Columns
        img.SetMetaData("0028|0030", f"{spacing}\\{spacing}")  # PixelSpacing
    ...
    print(f"Exported {len(recon_images)} DICOM files to {output_dir}")
...
def run_pipeline(
    ...
    save_nifti : bool
        Assemble slices and save as NIfTI volume in body space.
    save_dicom : bool
        Export slices as DICOM files.
    ...
    volume_path : str or None
        Path to assembled NIfTI volume if save_nifti=True.
    """
    ...
    # Assemble volume and save NIfTI
    if save_nifti and len(recon_images) > 0:
        print("Assembling slices into body-space volume...")
        recon_sitk = []
        for i, (recon, slice_spec) in enumerate(zip(recon_images, series_spec.slices)):
            slice_sitk = place_slice_in_body(
                recon, slice_spec, seq_fov_mm, config.slice_thickness_mm, rho_img
            )
            recon_sitk.append(slice_sitk)
        
        volume = assemble_volume(recon_sitk, rho_img)
        volume_path = os.path.join(output_dir, "volume.nii.gz")
        sitk.WriteImage(volume, volume_path)
        print(f"Volume saved to: {volume_path}")
    
    # Export DICOM
    if save_dicom and len(recon_images) > 0:
        dicom_dir = os.path.join(output_dir, "dicom")
        export_dicom_series(recon_images, series_spec, dicom_dir)
    
    return recon_images, kspace_list, volume_path
